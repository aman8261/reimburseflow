...
import { Settings, Lock, CheckCircle2, Circle } from 'lucide-react';
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'4px 16px', marginTop:6 }}>
        {[
          ...
          <div key={item.key} style={{display:'flex',alignItems:'center',gap:6,fontSize:'0.6875rem',color:checks[item.key as keyof typeof checks]?'#2ecc71':'#9ca3af'}}>
            {checks[item.key as keyof typeof checks]?<CheckCircle2 size={16}/>:<Circle size={16}/>}{item.label}
          </div>
        ))}
      </div>
    </div>
    ...
    <div>
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          <Settings size={28} color="#1f2937" />
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1f2937', margin: 0 }}>Settings</h1>
        </div>
      ...
      <div className="card" style={{ padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          ...
          <h3 style={{ fontSize: '1.125rem', fontWeight: 600, margin: 0, color: '#1f2937' }}>Change Password</h3>
        </div>

        {success && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', background: '#d4edda', color: '#155724', borderRadius: 8, marginBottom: 16, fontSize: '0.875rem' }}>
        ...
        <form onSubmit={handleChangePassword} style={{ maxWidth: 400 }}>
          ...
          <div className="form-group">
            ...
            {confirmPassword && newPassword === confirmPassword && confirmPassword.length > 0 && (
              <span style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 4, fontSize: '0.8125rem', color: '#2ecc71' }}><CheckCircle2 size={14} /> Passwords match</span>
            )}
          </div>
