...
import { ClipboardList, PercentSquare, User, Shuffle, Cog, Trash2, Key } from 'lucide-react';
...
export default function ApprovalRulesPage() {
  ...
  const ruleTypeLabels: Record<string, React.ReactNode> = {
    SEQUENTIAL: <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}><ClipboardList size={14} /> Sequential</span>,
    PERCENTAGE: <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}><PercentSquare size={14} /> Percentage</span>,
    SPECIFIC_APPROVER: <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}><User size={14} /> Specific Approver</span>,
    HYBRID: <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}><Shuffle size={14} /> Hybrid</span>,
  };
      ...
      {loading ? (
        ...
        <div className="card">
          <div className="empty-state">
            <div className="empty-icon" style={{ display: 'flex', justifyContent: 'center' }}><Cog size={48} /></div>
          ...
          {rules.map(rule => (
            <div className="card" key={rule.id}>
              <div className="card-header">
                ...
                <button className="btn btn-ghost btn-sm" style={{ color: '#e74c3c', display: 'flex', alignItems: 'center', gap: '4px' }} onClick={() => handleDelete(rule.id)}><Trash2 size={16} /> Delete</button>
              </div>
              <div className="card-body">
                ...
                {rule.specificApprover && (
                  <p style={{ marginTop: 12, fontSize: '0.8125rem', color: '#6b7280', display: 'flex', alignItems: 'center', gap: '6px' }}>
