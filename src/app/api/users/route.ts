...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (updateData.role === 'ADMIN' && targetUser.role !== 'ADMIN') {
      return NextResponse.json({ error: 'Cannot grant Admin role to existing users' }, { status: 403 });
    }
    if (targetUser.role === 'ADMIN' && updateData.role && updateData.role !== 'ADMIN') {
      return NextResponse.json({ error: 'Cannot change the role of the company Admin' }, { status: 403 });
    }
